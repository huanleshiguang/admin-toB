 ...
 * @LastEditTime: 2023-07-28 18:37:10
    ...
    <el-form ref="formRef" :model="userForm" :rules="rules" label-width="auto" label-position="right">
      ...
      <el-form-item label="用户角色" prop="userRoleIds">
        <el-select v-model="userForm.userRoleIds" multiple placeholder="请选择（可多选）">
          <el-option v-for="item in roleList" :key="item.id" :label="item.roleName" :value="item.id" @click="handleRoleSelected(item)"/>
        </el-select>
      </el-form-item>
    </el-form>
  </DialogLayout>
...
// 获取院区科室Tree
async function loadHospAreaDepTree() {
  try {
